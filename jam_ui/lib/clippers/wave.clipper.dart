...
class WaveClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    if (!reverse && !flip) {
      Offset firstEndPoint = Offset(size.width * .5, size.height - 40);
      Offset firstControlPoint = Offset(size.width * .25, size.height - 50);
      Offset secondEndPoint = Offset(size.width, size.height - 100);
      Offset secondControlPoint = Offset(size.width * .75, size.height - 40);
