// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'edit_vibes.controller.dart';

// **************************************************************************
// RiverpodGenerator
// **************************************************************************

String _$userEditVibesControllerHash() =>
    r'ad6147d7fe429507cb475cc9991ee91ef33fddcf';

/// See also [UserEditVibesController].
@ProviderFor(UserEditVibesController)
final userEditVibesControllerProvider = AutoDisposeAsyncNotifierProvider<
    UserEditVibesController, VibesCollection>.internal(
  UserEditVibesController.new,
  name: r'userEditVibesControllerProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$userEditVibesControllerHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$UserEditVibesController = AutoDisposeAsyncNotifier<VibesCollection>;
String _$categorizedVibesControllerHash() =>
    r'cc3e51e7468a1710e17978483d060499f12d3905';

/// See also [CategorizedVibesController].
@ProviderFor(CategorizedVibesController)
final categorizedVibesControllerProvider = AutoDisposeAsyncNotifierProvider<
    CategorizedVibesController, VibesCollection>.internal(
  CategorizedVibesController.new,
  name: r'categorizedVibesControllerProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$categorizedVibesControllerHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$CategorizedVibesController
    = AutoDisposeAsyncNotifier<VibesCollection>;
String _$searchVibesControllerHash() =>
    r'82b18e4c91f14cfde2e6bb3f5badef2e2e08d556';

/// See also [SearchVibesController].
@ProviderFor(SearchVibesController)
final searchVibesControllerProvider = AutoDisposeAsyncNotifierProvider<
    SearchVibesController, VibesCollection>.internal(
  SearchVibesController.new,
  name: r'searchVibesControllerProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$searchVibesControllerHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$SearchVibesController = AutoDisposeAsyncNotifier<VibesCollection>;
// ignore_for_file: type=lint
// ignore_for_file: subtype_of_sealed_class, invalid_use_of_internal_member, invalid_use_of_visible_for_testing_member
